...
const saveToken = async(token, userId, expires, type, blacklisted=false)=>{
    const tokenDoc = await MsqlToken.create({
        token,
        userId,
        expires,
        type,
...
const verifyToken = async(token, type)=>{
    ...
    const tokenDoc = await MsqlToken.findOne({token, type, userId: payload.sub, blacklisted: false});
    if(!tokenDoc){
...
const generateAuthTokens = async(user)=>{
    let refreshTokenDoc = await MsqlToken.findOne({ userId: user.id });
